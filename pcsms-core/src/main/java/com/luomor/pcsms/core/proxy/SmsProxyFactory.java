package com.luomor.pcsms.core.proxy;
...
import com.luomor.pcsms.api.proxy.Order;
import com.luomor.pcsms.api.proxy.SmsProcessor;
import com.luomor.pcsms.api.proxy.SuppotFilter;
import com.luomor.pcsms.api.proxy.aware.SmsBlendConfigAware;
import com.luomor.pcsms.api.proxy.aware.SmsConfigAware;
...
//        LinkedList<SmsProcessor> ownerProcessors = PROCESSORS.stream().filter(processor -> !shouldSkipProcess(processor,smsBlend)).collect(Collectors.toCollection(LinkedList::new));
    ...
    */
    ...
    private static SmsDao getSmsDaoFromFramework() {
        SmsDao smsDao;
        smsDao = getSmsDaoFromFramework("com.luomor.pcsms.javase.config.SESmsDaoHolder", "JavaSE");
        if (null != smsDao) {
            return smsDao;
        }
        smsDao = getSmsDaoFromFramework("com.luomor.pcsms.starter.holder.SpringSmsDaoHolder", "SpringBoot");
        if (null != smsDao) {
            return smsDao;
        }
        smsDao = getSmsDaoFromFramework("com.luomor.pcsms.solon.holder.SolonSmsDaoHolder", "Solon");
        if (null != smsDao) {
            return smsDao;
        }
